...
func LeadsupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/lead/leads/%s%s", cfg.BaseURL, id, queryString)
		...
		var result models.UpdateLeadResponse
...
func CreateLeadsupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_lead_leads_id",
		mcp.WithDescription("Update lead"),
		...
		mcp.WithString("lead_id", mcp.Description("Input parameter: The identifier of the lead.")),
		mcp.WithString("title", mcp.Description("Input parameter: The job title of the lead.")),
		mcp.WithString("first_name", mcp.Description("Input parameter: The first name of the lead.")),
		mcp.WithArray("tags", mcp.Description("")),
		mcp.WithString("created_at", mcp.Description("Input parameter: Date created in ISO 8601 format")),
		mcp.WithString("status", mcp.Description("")),
		mcp.WithArray("custom_fields", mcp.Description("")),
		mcp.WithString("description", mcp.Description("Input parameter: The description of the lead.")),
		mcp.WithString("prefix", mcp.Description("Input parameter: The prefix of the lead.")),
		...
		mcp.WithString("company_id", mcp.Description("Input parameter: The company the lead is associated with.")),
		mcp.WithString("monetary_amount", mcp.Description("Input parameter: The monetary amount of the lead.")),
		mcp.WithArray("websites", mcp.Description("")),
		mcp.WithString("owner_id", mcp.Description("Input parameter: The owner of the lead.")),
		...
		mcp.WithString("lead_source", mcp.Description("Input parameter: The source of the lead.")),
		mcp.WithArray("social_links", mcp.Description("")),
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: Full name of the lead.")),
		mcp.WithString("last_name", mcp.Description("Input parameter: The last name of the lead.")),
		mcp.WithString("updated_at", mcp.Description("Input parameter: Date updated in ISO 8601 format")),
		mcp.WithString("company_name", mcp.Required(), mcp.Description("Input parameter: The name of the company the lead is associated with.")),
		mcp.WithString("fax", mcp.Description("Input parameter: The fax number of the lead.")),
		mcp.WithString("language", mcp.Description("Input parameter: language code according to ISO 639-1. For the United States - EN")),
		mcp.WithString("id", mcp.Description("Input parameter: Unique identifier for the contact.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    LeadsupdateHandler(cfg),
	}
}
